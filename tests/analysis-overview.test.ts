...
test("analysis overview exposes persisted 7K scores and canonical archetype for the interactive result", () => {
  ...
  assert.deepEqual(overview.currentScoreArguments[0], {
    ...
    matchedCriterion: "Целостно понимает свои сильные стороны, ценности, принципы, ключевые смыслы и зачем занимается этой работой; может сформулировать основной смысл/миссию и понятно представить себя один на один. Это пока преимущественно внутреннее знание, а не регулярно используемый инструмент бизнеса.",
