...
export function validateDiagnosticInput(rawInput: unknown): DiagnosticInputV1_2 {
  const issues = collectSchemaIssues(rawInput, diagnosticInputSchema as JsonSchemaNode);
  if (issues.length > 0) throw new DiagnosticContractError(issues);
  const input = rawInput as DiagnosticInputV1_2;
  if (input.current.payingClientsCount !== null && input.current.clientsCountPeriod === null) {
    throw new DiagnosticContractError([
      {
