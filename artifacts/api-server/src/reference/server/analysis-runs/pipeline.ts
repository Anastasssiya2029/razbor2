import {
  analysisRunLocksTable as analysisRunLocks,
...
async function loadRun(analysisRunId: string): Promise<PipelineRunSnapshot | null> {
  ...
  return {
    ...
    input: validateDiagnosticInput(row.normalizedInput),
...
async function recoverInterruptedScoring(
  ...
  const recoveredStatus: AnalysisPipelineStatus = stored[0]?.result
    ? "targeting"
  ...
  await db
    .update(analysisRuns)
    .set({
      status: recoveredStatus as typeof analysisRuns.$inferInsert.status,
...
async function acquireLock(analysisRunId: string): Promise<PipelineLockAttempt> {
  ...
  const expiresAt = new Date(Date.now() + PIPELINE_LOCK_LEASE_SECONDS * 1000);
  const inserted = await db
    .insert(analysisRunLocks)
    .values({
      analysisRunId,
      lockedStage: run[0]?.status ?? "pipeline",
      lockToken: token,
      expiresAt,
    })
    .onConflictDoNothing({ target: analysisRunLocks.analysisRunId })
    .returning({ token: analysisRunLocks.lockToken });
  ...
  return {
    ...
    retryAfterSeconds: Math.max(
      1,
      Math.ceil(((active[0]?.expiresAt ?? new Date(Date.now() + 1000)).getTime() - now.getTime()) / 1000),
    ),
  };
}
...
async function renewLock(analysisRunId: string, token: string): Promise<boolean> {
  ...
  const renewed = await db
    ...
    .set({ expiresAt: new Date(Date.now() + PIPELINE_LOCK_LEASE_SECONDS * 1000) })
    .where(and(
      eq(analysisRunLocks.analysisRunId, analysisRunId),
      eq(analysisRunLocks.lockToken, token),
