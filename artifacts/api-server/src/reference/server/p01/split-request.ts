...
<EVIDENCE_ROUTING>
${promptJson({ version: EVIDENCE_ROUTING_RESOURCE_VERSION, elements: EVIDENCE_ROUTING, global: EVIDENCE_ROUTING_GLOBAL_CONTEXT })}
</EVIDENCE_ROUTING>
...
const CALIBRATION_ANCHORS: Record<SevenKElementId, string> = {
  ...
  product_method:
    "Фактически продающийся пакет или абонемент поддерживает уровень 3, отдельный комплексный флагман — уровень 4. Для уровня 5 путь А→Б, этапы, формат и результат не обязаны быть описаны одним связным текстом в одном поле: документированный кейс клиента (стартовая точка, что сделали, измеримый итог) из project.result вместе с описанием формата и цены флагмана и продающим сценарием его подачи из project.sales/project.clientPath вместе образуют этот путь. Число встреч и общее обещание результата сами по себе недостаточны, но конкретный кейс с измеримым до/после — достаточен. Сформулированный авторский метод, построенный на нём флагман и связная линейка (в том числе через даунсел/апсел между тарифами одного флагмана) могут поддерживать уровень 7; уровень 8 требует фактических переходов, повторных покупок или cross/up-sell.",
  sales_technology:
...
export type P01UpperLevelChallengeEnvelope = {
  elementId: SevenKElementId;
  nextLevelSatisfied: boolean;
  reason: string;
};

function upperLevelChallengeSchema(elementId: SevenKElementId): Record<string, unknown> {
  return {
    type: "object",
    additionalProperties: false,
    required: ["elementId", "nextLevelSatisfied", "reason"],
    properties: {
      elementId: { type: "string", enum: [elementId] },
      nextLevelSatisfied: { type: "boolean" },
      reason: { type: "string", minLength: 1, maxLength: 400 },
    },
  };
}

const validateChallengeSchemas = Object.fromEntries(
  SEVEN_K_ELEMENT_IDS.map((elementId) => [
    elementId,
    ajv.compile(upperLevelChallengeSchema(elementId)),
  ]),
) as Record<SevenKElementId, ReturnType<typeof ajv.compile>>;

export function upperLevelChallengeOutputSchema(
  elementId: SevenKElementId,
): Record<string, unknown> {
  return upperLevelChallengeSchema(elementId);
}

export function validateP01UpperLevelChallengeEnvelope(
  elementId: SevenKElementId,
  value: unknown,
): P01UpperLevelChallengeEnvelope {
  const validator = validateChallengeSchemas[elementId];
  if (!validator(value)) {
    throw new P01SchemaValidationError((validator.errors ?? []).map(schemaIssue));
  }
  return value as P01UpperLevelChallengeEnvelope;
}

/**
 * Independent second look at a single already-scored element. The scoring
 * prompt already asks the model to run this "upper-level challenge"
 * internally (see step 6 in `buildP01ElementScorePrompt`), but that
 * self-check is unverified: nothing stops the model from writing
 * `missing_evidence` that a fact already present in the ledger directly
 * contradicts. This pass re-asks the question in isolation, with only the
 * next level's rule and the evidence already routed to this element, so a
 * contradiction between the score and the ledger can be caught by the
 * backend (SR3-GLOBAL-DIRECT-HIGHER-EVIDENCE) instead of trusted blindly.
 */
export function buildP01UpperLevelChallengePrompt(options: {
  elementId: SevenKElementId;
  context: P01CoreContext;
  scorecard: P01ElementScore;
}): string | null {
  const { elementId, context, scorecard } = options;
  if (scorecard.score === null || scorecard.score >= 10) return null;
  const nextLevel = SCORING_RULES.elements[elementId].levels[scorecard.score + 1];
  ...
  return `# P-01 upper-level contradiction guard · ${elementId}

Ты — контролёр-методолог 7К. Элементу ${elementId} только что присвоили score=${scorecard.score} (${scorecard.matched_level_rule_id}). Проверь ровно один вопрос по правилу SR3-GLOBAL-DIRECT-HIGHER-EVIDENCE: удовлетворяет ли evidenceLedger, приложенный ниже, mandatoryCore либо одному alternativeEvidencePath СЛЕДУЮЩЕЙ ступени (score=${nextLevel.score}), без учёта boundarySignals текущей ступени?

Правила:
- Учитывай только evidenceLedger ниже; ничего не додумывай.
- Прямой current-факт высокого уровня имеет приоритет над отсутствием буквального описания промежуточных шагов (SR3-GLOBAL-DIRECT-HIGHER-EVIDENCE).
- Если evidence прямо противоречит следующей ступени (blockers ниже) или подтверждает только часть mandatoryCore, nextLevelSatisfied=false.
- Уже учтённые counterevidence_ids: ${promptJson(scorecard.counterevidence_ids)}.
- Уже заявленный missing_evidence текущего балла: ${promptJson(scorecard.missing_evidence)}. Если evidence ниже фактически закрывает один из этих пунктов, это признак nextLevelSatisfied=true.

<NEXT_LEVEL_RULE>
${promptJson({
  ruleId: nextLevel.ruleId,
  criterion: nextLevel.criterion,
  mandatoryCore: nextLevel.mandatoryCore,
  alternativeEvidencePaths: nextLevel.alternativeEvidencePaths,
  blockers: nextLevel.blockers,
  resilience: nextLevel.resilience,
})}
</NEXT_LEVEL_RULE>
<EVIDENCE_LEDGER>
${promptJson(evidence)}
</EVIDENCE_LEDGER>
Верни только JSON {"elementId":"${elementId}","nextLevelSatisfied":boolean,"reason":"краткое машинное обоснование, не для пользователя"} по schema.`;
}
