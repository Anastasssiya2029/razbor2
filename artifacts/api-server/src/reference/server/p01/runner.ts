...
export class P01RunExecutionError extends Error {
  ...
  constructor(options: {
    ...
    this.failureDetails = structuredClone(options.failureDetails ?? []) as P01ValidationIssue[];
  }
}
...
export async function runP01EvidenceScorer(
  ...
  async function runSplitCore(): Promise<P01RunOutcome> {
    ...
    async function challengeElement(
      context: P01CoreContext,
      elementId: SevenKElementId,
      scorecard: P01ElementScoreEnvelope["scorecard"],
    ): Promise<P01UpperLevelChallengeEnvelope | null> {
      const prompt = buildP01UpperLevelChallengePrompt({ elementId, context, scorecard });
      if (!prompt) return null;
      return completePart({
        key: `challenge.${elementId}`,
        schemaName: `p01_challenge_${elementId}_v1`,
        outputSchema: upperLevelChallengeOutputSchema(elementId),
        buildPrompt: () => prompt,
        validate: (value) => validateP01UpperLevelChallengeEnvelope(elementId, value),
      });
    }

    function upperLevelChallengeCorrection(
      elementId: SevenKElementId,
      scorecard: P01ElementScoreEnvelope["scorecard"],
      challenge: P01UpperLevelChallengeEnvelope,
    ): string {
      const nextLevel = SCORING_RULES.elements[elementId].levels[(scorecard.score ?? 0) + 1];
      return `Независимая backend-проверка (SR3-GLOBAL-DIRECT-HIGHER-EVIDENCE) подтвердила: evidenceLedger этого элемента прямо удовлетворяет mandatoryCore либо alternativeEvidencePath уровня ${nextLevel.score} (${nextLevel.ruleId}). Обоснование проверки: ${challenge.reason}\nЕсли ни один blocker или counterevidence этого уровня прямо этому не противоречит, пересчитай scorecard: score не ниже ${nextLevel.score}, matched_level_rule_id и next_level_rule_id обнови соответственно, evidence_ids и missing_evidence пересобери под подтверждённый уровень. Если противоречие всё же есть, оставь прежний score и явно перечисли его в missing_evidence.`;
    }
    ...
    // Unlike awaitAll, a rejection here does not fail the run: the
    // upper-level contradiction guard is an advisory re-check performed
    // after a validated score already exists, so a transient provider
    // failure on the challenge call must fall back to "not promoted"
    // rather than take down an otherwise-successful analysis.
    async function awaitAllAdvisory<T>(promises: Promise<T>[]): Promise<(T | null)[]> {
      const settled = await Promise.allSettled(promises);
    ...
    // Upper-level contradiction guard (task item 6): the scoring prompt already
    // asks the model to self-check score+1 internally, but that check is
    // unverified. Re-ask it in isolation for each element still below the
    // ceiling; if the ledger evidence directly satisfies the next level's
    // mandatoryCore/alternativeEvidencePath, force a re-score with that
    // finding attached. If the post-challenge merge fails invariants, keep
    // the already-validated pre-challenge result rather than failing the run.
    const challengeable = scoreEnvelopes.filter(
      ({ scorecard }) => scorecard.score !== null && scorecard.score < 10,
    );
    if (challengeable.length > 0) {
      const challenges = await awaitAllAdvisory(
        challengeable.map(({ elementId, scorecard }) => challengeElement(context, elementId, scorecard)),
      );
      const promotable = challengeable
        .map((envelope, index) => ({ envelope, challenge: challenges[index] }))
        .filter((item): item is { envelope: P01ElementScoreEnvelope; challenge: P01UpperLevelChallengeEnvelope } =>
          item.challenge !== null && item.challenge.nextLevelSatisfied,
        );
      if (promotable.length > 0) {
        const previousScoreEnvelopes = scoreEnvelopes;
        const previousResult = result;
        try {
          const replacements = await awaitAll(
            promotable.map(({ envelope, challenge }) => {
              reevaluationRetryCount += 1;
              return scoreElement(
                context,
                envelope.elementId,
                upperLevelChallengeCorrection(envelope.elementId, envelope.scorecard, challenge),
              );
            }),
          );
          const byElement = new Map(scoreEnvelopes.map((item) => [item.elementId, item]));
          ...
          scoreEnvelopes = previousScoreEnvelopes;
