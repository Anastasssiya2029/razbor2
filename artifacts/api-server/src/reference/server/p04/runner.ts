...
import { P04_PROMPT_VERSION } from "@/server/7k/prompts/p04.v1.3";
...
// A single semantic-invariant failure (e.g. the model rephrasing a fixed
// task with a new imperative verb) is common enough that giving up after one
// re-ask forces the manager into a slow, expensive manual "Повторить
// попытку" retry that hits the exact same failure class again. Allow a
// couple of extra corrective re-asks before surfacing P04_INVARIANT_FAILED.
const MAX_REEVALUATION_RETRIES = 2;
...
function correctionFor(title: string, issues: readonly P04ValidationIssue[]): string {
  return [
    ...
    "Верни весь JSON заново по schema 1.3. Не меняй upstream decisions, task IDs, route identities, businessValidation, Money Now status или locked teaser. Не добавляй новые действия. Используй только SOURCE_REGISTRY. Клиентские тексты перепиши коротко и разговорно, как понятное объяснение другу.",
  ].join("\n");
}
  ...
  while (true) {
    ...
    try {
      ...
      if (error instanceof P04SchemaValidationError) {
        ...
        if (technicalRetryCount === 0) {
          technicalRetryCount += 1;
          correction = correctionFor("Нарушена JSON Schema 1.3", error.issues);
          continue;
        }
      ...
      if (error instanceof P04InvariantError) {
        ...
        if (reevaluationRetryCount < MAX_REEVALUATION_RETRIES) {
