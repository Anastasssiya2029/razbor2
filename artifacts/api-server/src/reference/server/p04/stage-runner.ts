import { P04_PROMPT_SHA256, P04_PROMPT_VERSION } from "@/server/7k/prompts/p04.v1.3";
...
function mergeAttemptDiagnostics(
  ...
  if (!previous?.length) return structuredClone(current) as P04AttemptDiagnostic[];
...
export async function runP04Stage(
  ...
  try {
    ...
    const provider = options.provider
      ?? withCallLogging(createConfiguredP04Provider(process.env as Record<string, string | undefined>), {
        module: "p04",
        analysisRunId,
      });
