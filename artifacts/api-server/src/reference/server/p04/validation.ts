...
import p04OutputSchema from "@/schemas/p04-report-writer.output.v1.3.schema.json";
...
export class P04SchemaValidationError extends Error {
  ...
  constructor(readonly issues: P04ValidationIssue[]) {
    super("P-04 output does not satisfy schema 1.3");
...
function hydrateP04ImmutableSkeleton(value: unknown, input: P04PreparedInput): unknown {
  ...
  result.promptVersion = "P-04.v1.3";
...
function safeNarrativeTypography(value: string): string {
  return value
    ...
    .replace(/(^|[^\p{L}])будете\s+(?:готова|готов)(?=$|[^\p{L}])/giu, "$1будете готовы")
    .replace(
      /(^|[^\p{L}])(вы|она|он|клиентка|клиент|экспертка|эксперт)\s+(?:готова|готов)(?=$|[^\p{L}])/giu,
      (_match, boundary: string, subject: string) => `${boundary}${/^\p{Lu}/u.test(subject) ? "Вы" : "вы"} готовы`,
    )
    .replace(
      /(^|[^\p{L}])(вы|она|он|клиентка|клиент|экспертка|эксперт)\s+(?:сделала|сделал)(?=$|[^\p{L}])/giu,
      (_match, boundary: string, subject: string) => `${boundary}${/^\p{Lu}/u.test(subject) ? "Вы" : "вы"} сделали`,
    )
    .replace(
      /(^|[^\p{L}])(вы|она|он|клиентка|клиент|экспертка|эксперт)\s+(?:начала|начал)(?=$|[^\p{L}])/giu,
      (_match, boundary: string, subject: string) => `${boundary}${/^\p{Lu}/u.test(subject) ? "Вы" : "вы"} начали`,
    )
    .replace(
      /(^|[^\p{L}])(вы|она|он|клиентка|клиент|экспертка|эксперт)\s+(?:выстроила|выстроил)(?=$|[^\p{L}])/giu,
      (_match, boundary: string, subject: string) => `${boundary}${/^\p{Lu}/u.test(subject) ? "Вы" : "вы"} выстроили`,
    )
    .replace(
      /(^|[^\p{L}])(вы|она|он|клиентка|клиент|экспертка|эксперт)\s+(?:решила|решил)(?=$|[^\p{L}])/giu,
      (_match, boundary: string, subject: string) => `${boundary}${/^\p{Lu}/u.test(subject) ? "Вы" : "вы"} решили`,
    );
}
...
export function canonicalizeP04NarrativePresentation(
  ...
  result.routeCards = result.routeCards.map((card) => ({
    ...
    client_presentation: {
      items: card.client_presentation.items.map((item) => ({
        ...item,
        client_task: clean(item.client_task),
        client_done_when: clean(item.client_done_when),
      })),
    },
  }));
...
function authoredTexts(result: P04ResultV1_2): Array<{ path: string; value: string }> {
  ...
  result.routeCards.forEach((card, index) => {
    ...
    card.client_presentation.items.forEach((item, itemIndex) => {
      texts.push(
        { path: `/routeCards/${index}/client_presentation/items/${itemIndex}/client_task`, value: item.client_task },
        { path: `/routeCards/${index}/client_presentation/items/${itemIndex}/client_done_when`, value: item.client_done_when },
      );
    });
  });
...
function allSourceRefGroups(result: P04ResultV1_2): Array<{ path: string; refs: string[] }> {
  ...
  result.routeCards.forEach((item, index) => {
    groups.push({ path: `/routeCards/${index}/source_refs`, refs: item.source_refs });
    item.client_presentation.items.forEach((clientItem, itemIndex) => {
      groups.push({ path: `/routeCards/${index}/client_presentation/items/${itemIndex}/source_refs`, refs: clientItem.source_refs });
    });
  });
...
function validateNoNewTasks(
  ...
  input.context.resolvedPlan.cards.forEach((card, index) => {
    ...
    const resultCard = result.routeCards[index];
    resultCard?.client_presentation.items.forEach((item, itemIndex) => {
      const fixedTask = card.tasks.find((task) => task.taskId === item.task_id);
      const grounding = fixedTask ? [fixedTask.task] : tasks;
      routePaths.set(`/routeCards/${index}/client_presentation/items/${itemIndex}/client_task`, grounding);
      routePaths.set(`/routeCards/${index}/client_presentation/items/${itemIndex}/client_done_when`, grounding);
    });
  });
...
function validateClientPresentation(
  issues: P04ValidationIssue[],
  result: P04ResultV1_2,
  input: P04PreparedInput,
): void {
  input.context.resolvedPlan.cards.forEach((card, index) => {
    ...
    const items = resultCard.client_presentation.items;
    const path = `/routeCards/${index}/client_presentation/items`;
    if (items.length < 2 || items.length > 7) {
      add(issues, path, "client_checklist_size", "client_presentation.items must contain between 2 and 7 entries.");
    }
    const validTaskIds = new Set(card.tasks.map((task) => task.taskId));
    const tasksById = new Map(card.tasks.map((task) => [task.taskId, task]));
    items.forEach((item, itemIndex) => {
      const itemPath = `${path}/${itemIndex}`;
      if (!validTaskIds.has(item.task_id)) {
        add(issues, `${itemPath}/task_id`, "client_checklist_unknown_task", `task_id ${item.task_id} is not one of this card's immutable fixed tasks.`);
        return;
      }
      const fixedTask = tasksById.get(item.task_id)!;
      if (normalized(item.client_task) === normalized(fixedTask.task)) {
        add(issues, `${itemPath}/client_task`, "client_checklist_verbatim_copy", "client_task must not verbatim copy the canonical fixed task text.");
      }
      if (normalized(item.client_done_when) === normalized(fixedTask.doneWhen)) {
        add(issues, `${itemPath}/client_done_when`, "client_checklist_verbatim_copy", "client_done_when must not verbatim copy the canonical done_when text.");
      }
    });
  });
}
...
function validateTextInvariants(
  ...
  // Catches a causal (non-guarantee) claim that ties a score/metric change directly
  // to a ruble figure, e.g. "рост funnel с 6 до 8 даст ≈800 тыс дохода". P-03 never
  // computes real revenue deltas for this kind of claim (buildP03BackendRevenueScenario
  // always returns null), so any such sentence in the narrative is unbacked arithmetic
  // even though it does not use guarantee-style wording the incomePromise regex expects.
  const causalRevenueClaim = /с\s*\d+\s*до\s*\d+[^.!?]{0,60}?(?:даст|принес\p{L}*|добав\p{L}*|обеспеч\p{L}*|увелич\p{L}*|состав\p{L}*)[^.!?]{0,40}?(?:доход\p{L}*|выручк\p{L}*|денег|заработ\p{L}*|₽|руб\p{L}*)/iu;
  ...
  authoredTexts(result).forEach(({ path, value }) => {
    ...
    if (causalRevenueClaim.test(value)) add(issues, path, "income_promise", "P-04 cannot tie a score/metric change to a specific ruble figure; P-03 never computes real revenue deltas.");
...
export function validateP04Invariants(
  ...
  validateClientPresentation(issues, result, input);
