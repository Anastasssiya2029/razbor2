...
import { SEVEN_K_ELEMENT_IDS, type SevenKElementId } from '@/lib/server/7k/types';
import { archetypeDefinitions } from '@/lib/business-analysis';
import { growthRole, orderedGrowthElements, resolveGrowthPriorityPlan, type GrowthPriorityPlan } from '@/lib/growth-priority-plan';
...
function checklistRole(plan: GrowthPriorityPlan, elementId: SevenKElementId): string {
...
export function AnalysisPdfView({ result, managerPlan, deadlineLabel, currentRevenueRub, targetRevenueRub, currentWeeklyHours }: Props) {
  ...
  const checklistRank = (elementId: SevenKElementId) => {
    const index = checklistOrder.indexOf(elementId);
    return index === -1 ? Number.MAX_SAFE_INTEGER : index;
  };
  const checklistCards = applyManagerPlan(
    buildCanonicalChecklist(result, currentWeeklyHours),
    managerPlan,
    result.provenance.assemblyInputHash,
  ).slice().sort((left, right) => checklistRank(left.elementId) - checklistRank(right.elementId));
      ...
      <div className="analysis-pdf-page-content">
        ...
        <div className="analysis-pdf-archetype-grid">
          <section className="analysis-pdf-archetype-face">
            <img src={ARCHETYPE_IMAGES[archetypeId]} alt="" width={720} height={960} />
              ...
              <div className={`analysis-pdf-neuro-list ${specialistIds.length > 1 ? 'multiple' : ''}`}>
                {specialistIds.map((specialistId) => {
                  ...
                  return <article key={specialistId}>
                    <img src={specialist.image} alt="" width={240} height={240} />
