...
Не используй глобальный список причин и не предполагай, что причина разрешена
вообще. Допустимы только cause codes, фактически переданные backend в
scenario-specific `<MONEY_PRESCRIPTION_RULES>`.
...
Если доказано несколько причин, используй только порядок `causePrecedence`,
который backend передал для выбранного scenario. Не добавляй собственный порядок
и не применяй precedence без evidence.
...
Соответствующую разрешённую причину можно выбрать только если evidence показывает,
что бесплатный контакт уже даёт существенную часть законченного решения.

### Низкая цена
Низкая цена сама по себе не причина.
- прямой страх собственной цены и ограничивающая экономика продукта — разные
  механизмы; выбирай только тот код, который разрешён scenario rules и доказан.

### Повторных продаж нет
Причина про повторную продажу допустима только если есть клиенты/результат и
доказан реальный разрыв продолжения.
...
## 8. STRUCTURED INTERVENTION HISTORY REVIEW

Stage 7 уже проверил scenario-level history guard.
Не пересматривай selection по истории.

Но intervention-level повторение всё ещё запрещено. Для КАЖДОГО выбранного
intervention обязательно верни один объект в `interventionHistoryReview`.
Коды должны совпадать с выбранными interventions один-к-одному, без пропусков и
дублей.
...
- `conclusion`: `clear_to_test | blocked_repeat_without_new_condition | blocked_insufficient_history_evidence`.

Правила:
- `not_reported` — релевантной попытки в persisted attempts нет; matched/new
  evidence пусты; новое условие `not_applicable`; вывод `clear_to_test`;
- `no_match` — история есть, но выбранная интервенция её не повторяет; вывод
  `clear_to_test`;
- `matched` — обязательно сослаться на evidence конкретной persisted attempt;
  при `confirmed` добавить current evidence нового условия и вернуть
  `clear_to_test`; при `not_confirmed | unknown` вернуть
  `blocked_by_inconsistency`, null prescription/test/targetMetric и вывод
  `blocked_repeat_without_new_condition`;
- `unclear` — если нельзя безопасно определить смысловое совпадение, вернуть
  `blocked_by_insufficient_evidence`, null cause statement/prescription/test/targetMetric
  и вывод `blocked_insufficient_history_evidence`.

History review обязателен независимо от `sanityChecks`. Не жди, что backend
догадается о повторе по добровольному sanity check.
...
Каждый объект `<BACKEND_METRICS>` имеет `metric_code`, `role`, `value`, `unit`,
`source`, `evidence_ids`.

`baseline_value` можно использовать только через exact
`baseline_metric_code`, у которого `role = baseline | reference`.

`target_value` можно использовать только через exact `target_metric_code`, у
которого `role = target`. Baseline/reference нельзя копировать в numeric target.
...
Верни только JSON по `P03_OUTPUT_SCHEMA_V1_5`.
...
Для `blocked_by_insufficient_evidence` дополнительно:
- `primary_cause_code = null`;
- `cause_statement = null`;
- `contributing_cause_codes = []`.
