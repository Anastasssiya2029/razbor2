...
export function buildP01SystemPrompt(
  ...
  prompt += `Для каждого элемента построй накопительный портрет уровня: criterion — обязательное ядро, supporting dimensions могут быть покрыты примерно на 80%. Прямое противоречие ядру блокирует уровень; отсутствие упоминания является missing_evidence, а не counterevidence.\n`;
  ...
  prompt += `Проверочные опоры методологии: проявленный в бизнесе авторский способ плюс повторяемое подтверждение клиентов может поддерживать authenticity=7; признаки подходящего и неподходящего клиента могут поддерживать audience=6; сформулированный авторский метод и связная линейка могут поддерживать product_method=7, а уровень 8 требует фактических переходов или повторных продаж; переданный менеджерам полный цикл до оплаты может поддерживать sales_technology=9; масштабируемый источник, база, измеренный путь и экономика могут поддерживать funnel=7; масштабируемое привлечение целевой аудитории может поддерживать blog=6, а регулярные продажи из контента требуются для blog=7; переданные целые процессы с владельцами измеримого результата могут поддерживать team=6. Применяй эти опоры только при фактическом evidence, не как автоматические баллы.\n`;
