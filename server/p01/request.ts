...
export function buildP01SystemPrompt(
  ...
  prompt += `Для каждого элемента сначала проверь уровни 10→0 и выбери самый высокий уровень, чья ключевая бизнес-механика подтверждена. Соседние уровни — шкала зрелости, а не требование дословно повторить каждую формулировку всех нижних ступеней.\n`;
  ...
  prompt += `Проверочные опоры методологии: проявленный в продукте/упаковке/продажах авторский способ плюс повторяемое подтверждение клиентов может поддерживать authenticity=7; явные критерии «свой/не свой», готовности, запроса, результата и барьеров могут поддерживать audience=6; работающие переходы между входным, основным и следующими продуктами могут поддерживать product_method=8; переданная менеджерам технология встреч и переписки с контролем показателей может поддерживать sales_technology=9; платный источник, база, измеренный путь и экономика могут поддерживать funnel=7; контент, переводящий аудиторию в следующий шаг общей воронки, может поддерживать blog=6; переданные целые процессы с владельцами измеримого результата могут поддерживать team=6. Применяй эти опоры только при фактическом evidence, не как автоматические баллы.\n`;
