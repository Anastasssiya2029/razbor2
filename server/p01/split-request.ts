...
const CALIBRATION_ANCHORS: Record<SevenKElementId, string> = {
  ...
  product_method:
    "Сформулированный авторский метод, построенный на нём флагман и связная линейка могут поддерживать уровень 7; уровень 8 требует фактических переходов, повторных покупок или cross/up-sell.",
  sales_technology:
  ...
  blog:
    "Масштабируемая органическая или платная механика роста целевой аудитории может поддерживать уровень 6; регулярные обращения и оплаты из контента требуются для уровня 7.",
  team:
...
3. Построй накопительный портрет каждого уровня: criterion — обязательное ядро, supporting dimensions могут быть покрыты примерно на 80%. Прямой blocker обязательного ядра запрещает уровень; отсутствие упоминания является missing_evidence, а не counterevidence.
...
7. score не выше evidence_cap. Без конкретного current-примера cap≤2; только единичный случай или только история cap≤3; уровни 8–10 требуют повторяемости, результата, понимания причин и управляемости согласно правилам.
8. Используй максимум 5 strongest evidence_ids и только exact ID из ledger. Нельзя придумывать или переименовывать ID.
9. Проверочная опора: ${CALIBRATION_ANCHORS[elementId]} Это не автоматический балл — применяй только при подтверждённых фактах.
