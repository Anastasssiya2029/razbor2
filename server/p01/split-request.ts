...
const CALIBRATION_ANCHORS: Record<SevenKElementId, string> = {
  ...
  sales_technology:
    "Переданная менеджеру стандартная первая продажа до оплаты с контролем качества и рабочей заменой может поддерживать уровень 8; уровень 9 требует действующей системы повторных продаж, продлений, допродаж или реактивации и измерения повторной выручки и LTV.",
  funnel:
